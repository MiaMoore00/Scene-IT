        ...
        t.addEventListener('click', () => {
            if(movieGenres.length == 0){
                ...
                if(movieGenres.includes(genre.id)){
                    movieGenres.forEach((id, idx) => {
                        if(id == genre.id){
                            movieGenres.splice(idx, 1);
                        }
                    })
                }else{
                    movieGenres.push(genre.id);
            ...
            renderMovies(apiURL + '&with_genres='+encodeURI(movieGenres.join(',')))
            highlightSelection()
        })
        tagsEl.append(t);
    })
...
function openNav(movie) {
  let id = movie.id;
  fetch(baseUrl + '/movie/'+id+'/videos?'+myAPIkey).then(res => res.json()).then(videoData => {
    ...
    if(videoData){
      ...
      if(videoData.results.length > 0){
        ...
        overlayContent.innerHTML = `<h1 class="no-results">oh Em Gee! No Movies Found :( !</h1>`
      }
    }
  })
}
